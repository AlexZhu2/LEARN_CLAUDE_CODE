#!/usr/bin/env python3
"""
s06: Subagent — s05 hooks + 18 tools + task tool for context isolation.

  Parent Agent                           Subagent
  +------------------+                  +------------------+
  | messages=[...]   |                  | messages=[task]  | <-- fresh
  | tool: task       | ---------------> | own loop (30 max)|
  | result = summary | <--------------- | return last text |
  +------------------+                  +------------------+

  + All s05: todo_write, nag, plan-only, full hooks, 16 base tools
  + task tool + spawn_subagent() — subagent gets bash/read/write/edit/glob only
  + Subagent cannot spawn sub-subagents (no task in SUB_TOOLS)
...
# ═══════════════════════════════════════════════════════════
#  agent_loop — s04 hooks + s05 nag + s06 task auto-dispatch
# ═══════════════════════════════════════════════════════════
...
if __name__ == "__main__":
    print("s06: Subagent — s05 planning + spawn sub-agents with fresh context")
