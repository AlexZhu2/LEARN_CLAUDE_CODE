...
def log_tool_call(name: str, inputs: dict, output: str) -> None:
    """Print tool I/O to terminal; persist apply_patch input for inspection."""
    if name == "apply_patch":
        DEBUG_DIR.mkdir(exist_ok=True)
        patch = inputs.get("patch", "")
        (DEBUG_DIR / "last_apply_patch.txt").write_text(
            f"path={inputs.get('path', '')}\n\n{patch}", encoding="utf-8"
        )

    if output.startswith("Error"):
        print(f"\033[31m  ← {output[:800]}\033[0m")
    elif DEBUG:
        preview = json.dumps(inputs, ensure_ascii=False)
        if len(preview) > 400:
            preview = preview[:400] + "..."
        print(f"\033[90m  in: {preview}\033[0m")
        if name == "apply_patch":
            patch = inputs.get("patch", "")
            print(f"\033[90m  patch ({len(patch)} chars):\n{patch[:1000]}\033[0m")
        print(f"\033[90m  out: {output[:800]}\033[0m")
...
def run_append_file(path: str, content: str) -> str:
    ...
    print("使用了run_append_file工具")
    try:
        file_path = safe_path(path)
        file_path.parent.mkdir(parents=True, exist_ok=True)
        file_path.write_text(file_path.read_text(encoding="utf-8", errors="replace") + content, encoding="utf-8", errors="replace")
...
def run_safe_delete_file(path: str) -> str:
    ...
    """
    print("使用了run_safe_delete_file工具")
    try:
        file_path = safe_path(path)
...
def agent_loop(messages: list):
    """Run the agent loop: send messages to Claude, execute tool calls, append results.
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] == "--test-patch":
        sample = """<<<<<<< SEARCH
hello world
=======
hello patch
>>>>>>> REPLACE
"""
        blocks = parse_search_replace_blocks(sample)
        print(
            f"parser OK: {len(blocks)} block(s), search={blocks[0][0]!r}, replace={blocks[0][1]!r}"
        )
        test_file = DEBUG_DIR / "patch_test.txt"
        DEBUG_DIR.mkdir(exist_ok=True)
        test_file.write_text("hello world\n", encoding="utf-8")
        print(run_apply_patch(str(test_file.relative_to(WORKDIR)), sample))
        print(f"file now: {test_file.read_text(encoding='utf-8')!r}")
        sys.exit(0)

    if DEBUG:
        print("DEBUG=1: verbose tool logging enabled\n")
    else:
        print(
            "Tip: set DEBUG=1 for full tool input/output; apply_patch saves to .debug/last_apply_patch.txt\n"
        )
